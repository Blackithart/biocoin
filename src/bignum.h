...
/** C++ wrapper for BIGNUM (OpenSSL 1.1 / 3.x compatible) */
class CBigNum
{
    BIGNUM* bn;
    ...
    void allocate()
    {
        bn = BN_new();
        if (bn == NULL)
            throw bignum_error("CBigNum::allocate : BN_new failed");
    }
...
public:
    ...
    CBigNum(int8_t  n)  { allocate(); if (n >= 0) setuint32(n); else setint64(n); }
    CBigNum(int16_t n)  { allocate(); if (n >= 0) setuint32(n); else setint64(n); }
    CBigNum(int32_t n)  { allocate(); if (n >= 0) setuint32(n); else setint64(n); }
    CBigNum(int64_t n)  { allocate(); if (n >= 0) setuint64(n); else setint64(n); }

    CBigNum(uint8_t  n) { allocate(); setuint32(n); }
    CBigNum(uint16_t n) { allocate(); setuint32(n); }
    CBigNum(uint32_t n) { allocate(); setuint32(n); }
    ...
    */
    ...
    std::string ToString(int nBase=10) const
    {
        ...
        std::string str;
        CBigNum bnVal = *this;
        BN_set_negative(bnVal.bn, false);
        CBigNum dv;
        CBigNum rem;
        if (BN_cmp(bnVal.bn, bn0.bn) == 0)
            return "0";
        while (BN_cmp(bnVal.bn, bn0.bn) > 0)
        {
            if (!BN_div(dv.bn, rem.bn, bnVal.bn, bnBase.bn, pctx))
